...
# @added 20250611 - Branch #5630: v0.21.9
# cwt deprecated in scipy.signals
import pywt

import stumpy
from numpy.linalg import LinAlgError
# @modified 20250611 - Branch #5630: v0.21.9
#from scipy.signal import cwt, find_peaks_cwt, welch
...
def old_symmetry_looking(x, param):
    ...
    return pd.Series(
        {
            "value__symmetry_looking__r_{}".format(r["r"]): mean_median_difference
...
#    if np.abs(var) < 10 ** -10 or n == 1:
    if np.abs(var) < 10**-10 or n == 1:
        ...
        # @modified 20230105 - Branch #4456 - v0.19.1
        # Prior to v0.20.0 tsfresh passed the unbaised parameter, however unbiased changed to
        # adjusted in statsmodels v0.12.0
...
# Disabled the newly added root_mean_square algortihm added in blueyonder/tsfresh-v0.18.0
# @set_property("fctype", "simple")
# @set_property("minimal", True)
...
def original_pre_v090_fft_coefficient(x, c, param):
    ...
    return pd.Series(
        res, index=["{}__fft_coefficient__coeff_{}".format(c, q) for q in coefficients]
    )
...
# @modified 20250611 - Branch #5630: v0.21.9
# Variable changed from variance to var
...
def ar_coefficient(x, param):
    ...
    for parameter_combination in param:
        ...
        if k not in calculated_ar_params:
            try:
                # @modified 20230105 - Branch #4456 - v0.19.1
                # statsmodels deprecation of AR for AutoReg
...
def time_reversal_asymmetry_statistic(x, lag):
    ...
    if 2 * lag > n:
        ...
        return np.mean(
            (
                np.roll(x, 2 * -lag) * np.roll(x, 2 * -lag) * x
                - np.roll(x, -lag) * x * x
            )[0 : (n - 2 * lag)]
        )
...
# @added 20230105 - Branch #4456 - v0.19.1
# Added and disabled the new mean_n_absolute_max algorithm which was added in v0.19.0
# @set_property("fctype", "simple")
...
def lempel_ziv_complexity(x, bins):
    ...
    # @modified 20230105 - Branch #4456 - v0.19.1
    # AT THE MOMENT THIS SHOULD SAY not modified BUT NOTED
    # This is a change introduced in https://github.com/blue-yonder/tsfresh/commit/cb7943e57d949da546d6ea03259baa87a9f78a50
    # with the comments:
    # Fix conversion of time-series into sequence for lempel_ziv_complexity (#806)
    # Fix: First bin only contains minimum of input signal.
    ...
    # The original version pre v0.19.0
    bins = np.linspace(np.min(x), np.max(x), bins)
    # The new version as of v0.19.0, not enabled as needs testing
...
# @added 20230105 - Branch #4456 - v0.19.1
# Added and disabled the new absolute_maximum algorithm which was added in v0.19.0
@set_property("fctype", "simple")
@set_property("minimal", True)
# def absolute_maximum(x):
def v0190_absolute_maximum(x):
    """
...
# @added 20230105 - Branch #4456 - v0.19.1
# Added and disabled the v0170_benford_correlation algorithm which was added in v0.17.0 for diff alignment purposes
@set_property("fctype", "simple")
def v0170_benford_correlation(x):
    """
     Useful for anomaly detection applications [1][2]. Returns the correlation from first digit distribution when
     compared to the Newcomb-Benford's Law distribution [3][4].
